...
const Moviehome = ({ route }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (movieResponse?.data?.type != "places") {
          setMainRouteIndex("Challenges");
          setActiveRouteIndex("first");
        }
  ...
  const renderFood = () => (
    <ScrollView
      ...
      showsVerticalScrollIndicator={false}
    >
      {foodChallenge?.districtsData?.length > 0 &&
        foodChallenge.districtsData.map(
          (district, districtIndex) =>
            district.challenges?.length > 0 && (
              <View
                key={`breakfast-district-${districtIndex}`}
                style={styles.sectionContainer}
              >
                <SectionHeader title={`${district.title}`} />
                <FlatList
                  data={district.challenges}
                  keyExtractor={(item, index) =>
                    `breakfast-district-${districtIndex}-challenge-${index}`
                  }
                  horizontal
                  showsHorizontalScrollIndicator={false}
                  contentContainerStyle={styles.horizontalListContent}
                  renderItem={({ item, index }) => (
                    <FoodDisplay
                      item={item}
                      key={`breakfast-${districtIndex}-${index}`}
                      type="food"
                    />
                  )}
                />
  ...
  const renderExperience = () => (
    <ScrollView
      ...
      showsVerticalScrollIndicator={false}
    >
      {experienceChallenge?.districtsData?.length > 0 &&
        experienceChallenge.districtsData.map(
          (district, districtIndex) =>
            district.challenges?.length > 0 && (
              <View
                key={`breakfast-district-${districtIndex}`}
  ...
  const renderEvent = () => (
    ...
    >
      ...
      {trendingEvent?.districtsData?.length > 0 &&
        trendingEvent.districtsData.map(
          (district, districtIndex) =>
            district.challenges?.length > 0 && (
              <View
                key={`breakfast-district-${districtIndex}`}
