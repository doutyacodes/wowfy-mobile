...
// Tab Routes
const LeaderboardTab = ({ data, navigation, title, isLoading }) => {
    ...
    <View style={styles.tabContainer}>
      <View style={styles.tabHeader}>
        <Text style={styles.participantsText}>
          {data?.length > 0 ? data?.length : 0} Participants
        </Text>
...
const LeaderScreen = ({ route }) => {
  ...
  useEffect(() => {
    if (isFocused && user?.id) {
      ...
      queryClient.invalidateQueries(['weeklyLeaderboard', pageId, user.id]);
      queryClient.invalidateQueries(['monthlyLeaderboard', pageId, user.id]);
      queryClient.invalidateQueries(['yearlyLeaderboard', pageId, user.id]);
    }
  ...
  const { 
    data: weeklyLeaderboard = { data: [], single: {} }, 
    isLoading: isWeeklyLoading 
  } = useQuery({
    queryKey: ['weeklyLeaderboard', pageId, user?.id],
    queryFn: async () => {
      const response = await axios.get(
        `${baseURL}/getUserLeader.php?user_id=${user.id}&timeline=weekly&page_id=${pageId}`
      );
      return {
  ...
  const { 
    data: monthlyLeaderboard = { data: [], single: {} }, 
    isLoading: isMonthlyLoading 
  } = useQuery({
    queryKey: ['monthlyLeaderboard', pageId, user?.id],
    queryFn: async () => {
      const response = await axios.get(
  ...
  const { 
    data: yearlyLeaderboard = { data: [], single: {} }, 
    isLoading: isYearlyLoading 
  } = useQuery({
    queryKey: ['yearlyLeaderboard', pageId, user?.id],
    queryFn: async () => {
      const response = await axios.get(
  ...
  const isLoading = isPageLoading || !user?.id;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {(weeklyLeaderboard.single?.ranking || monthlyLeaderboard.single?.ranking || yearlyLeaderboard.single?.ranking) && (
        <View style={styles.userRankCard}>
          <Text style={styles.userRankTitle}>Your Ranking</Text>
          
          <View style={styles.userRankInfo}>
            {index == 0 && weeklyLeaderboard.single?.ranking && (
              <View style={styles.userRankItem}>
                <Text style={styles.userRankPosition}>#{weeklyLeaderboard.single.ranking}</Text>
                <Text style={styles.userRankPoints}>{weeklyLeaderboard.single.total_points} pts</Text>
                <Text style={styles.userRankTimeframe}>Weekly</Text>
              </View>
            )}
            
            {index == 1 && monthlyLeaderboard.single?.ranking && (
              <View style={styles.userRankItem}>
                <Text style={styles.userRankPosition}>#{monthlyLeaderboard.single.ranking}</Text>
                <Text style={styles.userRankPoints}>{monthlyLeaderboard.single.total_points} pts</Text>
                <Text style={styles.userRankTimeframe}>Monthly</Text>
              </View>
            )}
            
            {index == 2 && yearlyLeaderboard.single?.ranking && (
              <View style={styles.userRankItem}>
                <Text style={styles.userRankPosition}>#{yearlyLeaderboard.single.ranking}</Text>
                <Text style={styles.userRankPoints}>{yearlyLeaderboard.single.total_points} pts</Text>
                <Text style={styles.userRankTimeframe}>Yearly</Text>
              </View>
            )}
          </View>
        </View>
